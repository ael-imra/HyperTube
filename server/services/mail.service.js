...
const mailContent = async function (userName, subject, url,token) {
    ...
    return data.toString().replace("${token}", token).replace("${userName}", userName).replace("${message}", needed[subject].message).replace("${button}", needed[subject].button).replace("${url}", url)
}
