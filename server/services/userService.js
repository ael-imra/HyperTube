  ...
  if (inputs instanceof Object) {
    for (const key in inputs)
      if (properties.indexOf(key) === -1)
        return { Eng: `Error unsupported key ${key}`, Fr: `Erreur clé non prise en charge ${key}` };
    for (const key of properties) {
      if (Object.keys(inputs).indexOf(key) === -1)
        return { Eng: 'Error missing some inputs', Fr: 'Erreur manquant certaines entrées' };
...
const getJWT = async function (userID) {
  ...
  const jwt = sign({ userID }, process.env.KEY_JWT);
