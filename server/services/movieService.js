...
const getMovieImagesAndCasts = async function (imdbID) {
  try {
    const images = await axios.get(
      `https://api.themoviedb.org/3/movie/${imdbID}/images?api_key=${process.env.KEY_THEMOVIEDB}`
    );
    const credits = await axios.get(
      `https://api.themoviedb.org/3/movie/${imdbID}/credits?api_key=${process.env.KEY_THEMOVIEDB}`
    );
...
const getMovieInfo = async function (imdbID) {
  try {
    const movie = await axios.get(`https://api.themoviedb.org/3/movie/${imdbID}?api_key=${process.env.KEY_THEMOVIEDB}`);
    return movie.data;
  } catch (err) {
    return {};
  }
