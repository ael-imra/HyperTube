...
const getMovieDBInfo = async function (imdbID) {
  const [movieFromViewed] = await query('SELECT imdbID,movieTitle,movieRating,movieImage,movieLanguage,movieRelease,movieTime,movieGenre,movieDescription FROM Viewed WHERE imdbID = ? LIMIT 1', imdbID);
  if (movieFromViewed) return movieFromViewed;
  const [movieFromFavorite] = await query('SELECT imdbID,movieTitle,movieRating,movieImage,movieLanguage,movieRelease,movieTime,movieGenre,movieDescription FROM Favorites WHERE imdbID = ? LIMIT 1', imdbID);
