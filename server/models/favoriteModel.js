...
const getFavorites = async function (userID, justImdbID) {
  const needed = justImdbID ? justImdbID : "favoriteID,imdbID,movieTitle,movieImage,movieLanguage,movieRelease,movieTime,movieGender,movieDescription,(SELECT COUNT(userID) FROM Viewed WHERE userID=f.userID AND imdbID=f.imdbID) AS 'watched'";
