...
const getComments = async function (imdbID, userID) {
	const comments = await query(
		`SELECT IF(u.userID = ?,1,0) AS 'myComment',commentID,commentContent,date,u.userName,image FROM Comments c,Users u WHERE (u.userID=c.userID OR u.42ID=c.userID OR u.githubID=c.userID OR u.googleID=c.userID) AND imdbID=? ORDER BY date DESC`,
		[userID, imdbID]
	)
	return comments
}
