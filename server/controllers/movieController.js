...
const getMovieInfoBackup = async function (req, res, next) {
	try {
		...
		const torrents = []
		movieInfo.items.map((item, index) => (index <= 5 ? torrents.push({ quality: item.quality, hash: item.id, type: '' }) : null))
		return res.send({
			type: 'success',
			status: 200,
			body: {
