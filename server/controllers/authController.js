    ...
    if (user) {
      if (user.userID.indexOf('go') > -1 || user.userID.indexOf('gi') > -1 || user.userID.indexOf('42') > -1)
        return res.send({
          ...
          status: 403,
          body: {
            Eng: "You can't login with this username",
            Fr: "Vous ne pouvez pas vous connecter avec ce nom d'utilisateur",
          },
        });
  ...
  try {
    ...
    if (user & (user.userFrom !== 'local'))
      return res.send({
        ...
        status: 403,
        body: {
          Eng: 'Cannot reset password for this account',
          Fr: 'Impossible de réinitialiser le mot de passe pour ce compte',
        },
      });
    if (user) {
      if (!user.isActive)
        return res.send({
          ...
          status: 403,
          body: {
            Eng: 'Active your account than reset password',
      ...
      return res.send({
        ...
        body: {
          ...
          Fr: 'Vérifiez votre e-mail pour réinitialiser le mot de passe',
        },
      });
    }
    return res.send({
