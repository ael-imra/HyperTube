...
const {getUser, updateUser, getAllUsers} = require(__dirname +
  '/../models/userModel')
...
const getMyProfile = async function (req, res, next) {
  try {
    const user = await getUser({userID: req.user}, [
      'userFrom',
      'userName',
      'email',
      'firstName',
...
const editProfile = async function (req, res, next) {
  try {
    const error = checkUserInput(req.body, [
      'userName',
      'email',
      'firstName',
      'lastName',
    ...
    for (const key in req.body)
      if (req.body[key] !== user[key]) listNeedUpdate[key] = req.body[key]
...
const editPassword = async function (req, res, next) {
  try {
    const errorNewPassword = checkUserInput({password: req.body.newPassword}, [
      'password',
    ])
    const errorOldPassword = checkUserInput({password: req.body.oldPassword}, [
      'password',
    ])
...
const editImage = async function (req, res, next) {
  try {
    ...
    if (resultUpdate) {
      if (user.image && fs.existsSync(__dirname + '/..' + user.image))
