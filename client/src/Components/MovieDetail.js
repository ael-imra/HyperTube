        ...
        {movieInfo.torrents.map((movie, key) =>
          quality === key ? (
            <video width='100%' height='700px' controls crossOrigin='use-credentials' key={key}>
              ...
              <track label='English' kind='subtitles' srcLang='en' src={`http://localhost:1337/subtitle/${movieInfo.imdbCode}/en`} default={ctx.Lang === "Eng" ? true : false} />
              <track label='French' kind='subtitles' srcLang='fr' src={`http://localhost:1337/subtitle/${movieInfo.imdbCode}/fr`} default={ctx.Lang === "Fr" ? true : false} />
            </video>
          ) : (
            ""
          )
