      ...
      const detailPopularMovies = await new Promise((resolve) => {
        ...
        popularMovies.data.results.map(async (movie) => {
          const image = await Axios.get(
            `https://api.themoviedb.org/3/movie/${movie.id}/images?api_key=7a518fe1d1c5359a4929ef4765c347fb`
          );
          const getImdbCode = await Axios.get(
            `https://api.themoviedb.org/3/movie/${movie.id}/external_ids?api_key=7a518fe1d1c5359a4929ef4765c347fb`
          );
          ...
          result.push({
            ...
            isFavorite:
              listFavorite.data.body instanceof Array &&
              listFavorite.data.body.findIndex((a) => a.imdbID === getImdbCode.data.imdb_id) !== -1
                ? true
                : false,
